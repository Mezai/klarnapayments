	...
	public function buildGoodsList($cart, $klarna)
	{
		...
		foreach ($products as $product)
		{
			...
			try {
				...
				$klarna->klarna->addArticle($product['cart_quantity'],
				...
				Klarna::printDebug('error adding article -  '.$e->getMessage(), $product);
			}
		}
		// Add discounts
		...
		foreach ($discounts as $discount)
		{
			...
			try {
				...
				Klarna::printDebug('error adding article -  '.$e->getMessage(), $discount);
			}
		}
		...
		if ($shipmentfee > 0.0)
		{
			...
			$carried_loaded = Validate::isLoadedObject($carrier);
			if (!$carried_loaded)
			...
			if ($carrier->active)
			{
				$taxrate = Tax::getCarrierTaxRate((int)$carrier->id,
				...
				try {
					...
					Klarna::printDebug('error adding shipping fee - '.$e->getMessage(), $carrier);
				}
			}
		}
		...
		if ($cart->gift == 1)
		{
			try {
				...
				Klarna::printDebug('error adding giftwrap fee', $e->getMessage());
			}
		}
