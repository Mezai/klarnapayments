	...
	public function initContent()
	{
		...
		try
		{
			...
			if ($klarnaorder['status']  === "checkout_complete")
			{
				...
				VALUES ('.(int)$this->module->currentOrder.', \''.pSQL($reservation_number).'\', \''.pSQL($billing['given_name']).'\', \''.pSQL($billing['family_name']).'\', \''.pSQL($klarnaorder['status']).'\', \''.pSQL(Tools::strtoupper($shipping['country'])).'\')');
